  ...
  _runReleaseStep: function (step, params, callback) {
    ...
    async.eachSeries(this.releaseCommands, function _runReleaseCommandFn (releaseCommand, cb) {
      ...
      if (releaseCommand[step]) {
        // Prepare our stdio
        var stdin = that.options.stdin || process.stdin;
        var stdout = that.options.stdout || process.stdout;
        var stderr = that.options.stderr || process.stderr;

        // Perform fake variable replacement
        // TODO: Maybe use this instead...
        // https://github.com/npm/npm/blob/master/lib/utils/lifecycle.js#L210-L217

        // Nah, let's not be clever. Let's use minstache and call it a day
        ...
        // DEV: We need to pipe since a buffer isn't a valid file descriptor to pass off
        // TODO: See if there modules that help us conditionally inherit or not
        var child = spawn(cmd, cmdArgs, {stdio: 'pipe', env: cmdEnv});
        child.stdin.pipe(stdin);
        child.stdin.pipe(stdout);
        child.stdin.pipe(stderr);
