  ...
  releaseCommands = releaseCommands.map(function upcastReleaseCommand (releaseCommand) {
    if (releaseCommand.type === 'releaseCommand') {
      return _.defaults({}, releaseCommand, {
        updateFiles: [releaseCommand.command, 'update-files', '$FOUNDRY_VERSION', '$FOUNDRY_MESSAGE'],
        commit: [releaseCommand.command, 'commit', '$FOUNDRY_VERSION', '$FOUNDRY_MESSAGE'],
        publish: [releaseCommand.command, 'publish', '$FOUNDRY_VERSION', '$FOUNDRY_MESSAGE'],
        register: [releaseCommand.command, 'register', '$FOUNDRY_VERSION', '$FOUNDRY_MESSAGE']
      });
    } else {
      return releaseCommand;
    }
