...
function Release(options) {
  ...
  // TODO: Deprecate `moduleDir` as soon as we can. Maybe just don't document it.
  // TODO: It is too single purposed and a glob could do a much better job.
  options = options || {};
  this.moduleDir = options.moduleDir;
  console.log('hai', options.moduleDir);
}
...
extend(Release.prototype, {
  ...
  getReleaseLibs: function (cb) {
    ...
    npm.load(function (err) {
      ...
      glob('*/package.json', {cwd: that.moduleDir || npm.globalDir}, function (err, packages) {
        // If there was an error, callback with it
        if (err) {
          return cb(err);
        }
        ...
        console.log(releaseDirs);
        releaseDirs.push('./release/bower', './release/component', './release/git', './release/python');
