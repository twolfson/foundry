  ...
  _runReleaseStep: function (step, params, callback) {
    ...
    async.eachSeries(this.releaseCommands, function _runReleaseCommandFn (releaseCommand, cb) {
      ...
      if (releaseCommand[step]) {
        // Replace the variables in our command
        var stepParts = releaseCommand[step].map(function replaceVariables (str) {
          return str.replace(FOUNDRY_VERSION_VAR, params.version).replace(FOUNDRY_MESSAGE_VAR, params.message);
        });

        // Log the step we are running
        var stdout = that.options.stdout || process.stdout;
        stdout.write('Running step: ' + quote(stepParts) + '\n');

        // Prepare and run our command
        var cmd = stepParts[0];
        var cmdArgs = stepParts.slice(1);
        ...
        var child = spawn(cmd, cmdArgs, {stdio: stdio});
