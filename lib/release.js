...
Release.prototype = {
  ...
  getReleaseSteps: function (params) {
    ...
    if (params.FOUNDRY_VERSION === '1.0.0') {
      addSteps('register');
    }
    addSteps('publish');
  ...
  release: function (version, callback) {
    ...
    async.series([
      ...
      function notifyUserReleaseSteps (cb) {
        ...
        stdout.write('Configuring steps with ' + chalk.bold('FOUNDRY_VERSION') + ': ' + params.FOUNDRY_VERSION + '\n');
        stdout.write('Configuring steps with ' + chalk.bold('FOUNDRY_MESSAGE') + ': ' + params.FOUNDRY_MESSAGE + '\n');
        process.nextTick(cb);
      },
  ...
  _runReleaseSteps: function (params, callback) {
    ...
    async.eachSeries(params.steps, function _runReleaseStepFn (stepInfo, cb) {
      ...
      var cmdEnv = _.defaults({
        ...
        FOUNDRY_MESSAGE: params.FOUNDRY_MESSAGE
      }, process.env);
