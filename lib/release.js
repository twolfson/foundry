...
Release.prototype = {
  ...
  _runReleaseSteps: function (params, callback) {
    ...
    async.eachSeries(params.steps, function _runReleaseStepFn (stepInfo, cb) {
      ...
      if (err) {
        that.outputResumeFile(params, function handleOutputResumeFile (err2) {
          // If there was a secondary error, write it to stderr
          if (err2) {
            console.error(err2);
          }

          // TODO: Should there be a prompt for the user with recovery info?
          //   Maybe yet another error?
