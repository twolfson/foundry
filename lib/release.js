...
function Release(releaseLibs, options) {
  ...
  releaseLibs.forEach(function assertReleaseLibSpec (releaseLib) {
    ...
    assert.notEqual(specVersion, undefined, '`releaseLib.specVersion` is not defined in one of the release plugins for `foundry`. Please determine which one is out of date and update it.');
  });
