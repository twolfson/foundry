...
function Release(releaseLibs, options) {
  ...
  releaseLibs.forEach(function assertReleaseLibSpec (releaseLib) {
    ...
    var releaseLibName = (releaseLib.meta || {}).name;

    var undefinedSpecVersionMsg = releaseLibName !== undefined ?
        '`releaseLib.specVersion` is not defined in "' + releaseLibName + '". Please install a matching version.' :
        '`releaseLib.specVersion` is not defined in one of the release plugins for ' +
          '`foundry`. Please determine which one is out of date and install a matching version.';
    assert.notEqual(specVersion, undefined, undefinedSpecVersionMsg);

    var invalidSpecVersionMsg = releaseLibName !== undefined ?
          '`releaseLib.specVersion` is not a valid semver for "' + releaseLibName + '". Please install a valid version.' :
          '`releaseLib.specVersion` is not a valid semver for one of the release plugins ' +
            'for `foundry`. Please determine which one is invalid and install a valid version.';
    assert(semver.valid(specVersion), invalidSpecVersionMsg);

    var unsupportSpecVersionMsg = releaseLibName !== undefined ?
          'Expected: ">=' + Release.specVersion + '", Actual: "' + specVersion + '". `' + releaseLibName + '.specVersion` is below ' +
            'the required semver for `foundry`. Please install a supported version.' :
          'Expected: ">=' + Release.specVersion + '", Actual: "' + specVersion + '". `releaseLib.specVersion` is below ' +
            'the required semver for `foundry`. Please determine which library is out of date and install a supported version.';
    assert(semver.gte(specVersion, Release.specVersion), unsupportSpecVersionMsg);
  });
