  ...
  releaseCommands = releaseCommands.map(function upcastReleaseCommand (releaseCommand) {
    if (releaseCommand.type === 'releaseCommand') {
      return _.defaults({}, releaseCommand, {
        updateFiles: quote([releaseCommand.command, 'update-files', '"' + FOUNDRY_VERSION_VAR + '"', '"' + FOUNDRY_MESSAGE_VAR + '"']),
        commit: quote([releaseCommand.command, 'commit', '"' + FOUNDRY_VERSION_VAR + '"', '"' + FOUNDRY_MESSAGE_VAR + '"']),
        publish: quote([releaseCommand.command, 'publish', '"' + FOUNDRY_VERSION_VAR + '"', '"' + FOUNDRY_MESSAGE_VAR + '"']),
        register: quote([releaseCommand.command, 'register', '"' + FOUNDRY_VERSION_VAR + '"', '"' + FOUNDRY_MESSAGE_VAR + '"'])
      });
    } else {
      return releaseCommand;
    }
...
Release.prototype = {
  ...
  _getStdout: function () {
    return that.options.stdout || process.stdout;
  },
