...
extend(Foundry.prototype, {
  ...
  bindCommands: function () {
    ...
    // TODO: This should expand into its own subcommand set (e.g. `foundry plugin`, `foundry plugin install`)
    program
      .command('plugin')
      .description('List installed `foundry` plugins')
      .action(function listPlugins (argv) {
        // Gather release directories
        Foundry.getReleaseLibs({}, function handleReleaseDirs (err, releaseLibs) {
          // If there was an error, throw it
          if (err) {
            throw err;
          }
          ...
          console.log('Installed `foundry` plugins:');
          releaseLibs.forEach(function outputReleaseLib (releaseLib) {
            var moduleName = path.basename(releaseLib);
